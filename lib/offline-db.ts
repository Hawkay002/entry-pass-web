...
/** Replace the cached ticket snapshot (called whenever the realtime listener fires). */
...
export async function getCachedTickets(): Promise<Ticket[]> {
  try {
    const val = await tx<Ticket[]>(STORE_KV, "readonly", (s) =>
